...
vec2 panelMin=vec2(.176,.285);vec2 panelMax=vec2(.258,.585);
if(screenAspect<.9){panelMin=vec2(.505,.30);panelMax=vec2(.565,.55);}
vec2 panel=(st-panelMin)/(panelMax-panelMin);
panel.x+=(panel.y-.5)*.055;
float inside=step(0.,panel.x)*step(panel.x,1.)*step(0.,panel.y)*step(panel.y,1.)*billboardReady;
...
float frameGlow=exp(-abs(edge)*120.)*billboardReady;
